...
@DatasetReader.register("addresses_ner")
class AtcNerDatasetReader(DatasetReader):
    """
    Reads instances from a pretokenised file where each line is in the following format:
    WORD NER-TAG
    with a blank line indicating the end of each sentence
    and '-DOCSTART- O' indicating the end of each article,
    ...
    coding_scheme: ``str``, optional (default=``IOB1``)
        ...
        the original IOB1 scheme in the ATC NER (CoNLL 2003) NER data.
    ...
    def text_to_instance(
        ...
        #     coded_ner = to_bioul(
        #         ner_tags, encoding=self.
        #         _original_coding_scheme) if ner_tags is not None else None
        # else:
        #     # the default IOB1
        #     coded_ner = ner_tags
        coded_ner = ner_tags
